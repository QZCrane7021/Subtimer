...
public unsafe class MediaEngine : IDisposable
{
    ...
    // 音频处理
    private AVFormatContext* _audioFormatCtx;       // 音频处理上下文
    private short[] _pcmData;                       // PCM 缓存
    private int _sampleRate = 44100;                // 采样率
    private int _channels = 2;                      // 通道数（双声道）
    private long _currentSampleIndex = 0;           // 目前的采样点位
    public short[] PcmData => _pcmData;             // PCM 缓存对外暴露
    private int _pushStreamHandle;                  // 音频推流句柄

    // 有关音频区间播放的内容
    private double _rangeStartTime = 0;                     
    private double _rangeEndTime = 0;                       
    private bool _isRangePlaying = false;                   
    private int _playEndSyncHandle = 0;             // 播放结束同步句柄
    ...
    // 音视频同步
    private DispatcherTimer? _videoSyncTimer;   // 视频同步计时器
    private System.Timers.Timer _feedTimer;     // 音频推送计时器
    ...
    public void InitVideo(string filePath)
    {
        ...
        // 在 UI 线程创建对应尺寸的 WriteableBitmap 传给 UI
    ...
    private bool SeekVideoFrame(double seconds)
    {
        ...
        int maxFramesToDecode = 300; // 阻断计数器，防止脏包死循环
